...
"""GPU/CPU-optimized inference CLI for OmniVoice using the OpenVINO runtime.
...
audio decode) stays in PyTorch. 

Usage:
    # Voice design on GPU (Default)
    omnivoice-infer-openvino --model k2-fsa/OmniVoice \
        --ir openvino_ir/omnivoice-step-int8.xml \
        --text "Hello, this is running on my Intel GPU via OpenVINO." \
...
def install_openvino_forward(
    ...
    core = ov.Core()
    logging.info(f"Reading OpenVINO IR for target device [{device}]: {ir_path}")
    base_model = core.read_model(str(ir_path))

    # GPU-specifikus optimalizációs beállítások (CPU pinning és szálkezelés törölve)
    compile_kwargs = {
        "INFERENCE_PRECISION_HINT": "f32",
        "PERFORMANCE_HINT": "LATENCY",
    }

    compiled = core.compile_model(base_model, device, compile_kwargs)
    ...
    def forward(
        ...
        # Statikus formára igazítás. Első futásnál GPU-n ez eltarthat 1-2 percig (Kernel kompiláció)!
        if static_reshape and state["shape_seen"] is None:
            ...
            new_compiled = core.compile_model(new_model, device, compile_kwargs)
            state["request"] = new_compiled.create_infer_request()
            state["compiled"] = new_compiled
            state["out_port"] = new_compiled.outputs[0]
            logging.info(
                f"Recompiled for [{device}] with static shape B={B} S={S} "
                f"in {time.perf_counter() - t0:.2f}s (This is a one-time cost)"
            )
...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="OmniVoice single-item inference (OpenVINO GPU/CPU backend)",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the OpenVINO IR (.xml) for the diffusion-LM step.",
    )
    parser.add_argument(
        "--text",
        type=str,
    ...
    parser.add_argument(
        ...
        default="GPU",
        choices=["GPU", "CPU"],
        help="Target device for OpenVINO execution (GPU or CPU).",
    )
    parser.add_argument(
        "--static_reshape",
        type=str2bool,
        default=True,
        help="Recompile with static shapes on the first forward pass.",
    )
    return parser
...
def main():
    ...
    # Modell betöltve üzenet, majd 2 másodperc várakozás
    ...
    # Kezdési idő rögzítése és kiírása
    ...
    # Befejezési idő rögzítése és kiírása
    ...
    logging.info(
        ...
        f"OpenVINO device={args.device}, steps={stats['n_calls']}, mean={mean_step_ms:.1f} ms"
    )
